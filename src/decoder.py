...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, enc_output: torch.Tensor, tgt_mask: torch.Tensor) -> torch.Tensor:
        ...
        attention_output = self.self_attention(hidden_state, hidden_state, hidden_state, tgt_mask)
        ...
        x = cross_attention_output + x
        
        # Apply layer normalization and feed-forward network
        ff_output = self.feed_forward(self.layer_norm_3(x))
        x = ff_output + x
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor, enc_output: torch.Tensor) -> torch.Tensor:
        ...
        tgt_mask = torch.tril(torch.ones(seq_len, seq_len, device=input_ids.device)).unsqueeze(0).repeat(batch_size, 1, 1)
